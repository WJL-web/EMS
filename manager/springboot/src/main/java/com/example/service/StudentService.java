...
public class StudentService {
    ...
    public void add(Student student) {
        ...
        if (ObjectUtil.isEmpty(student.getPassword())) {
            student.setPassword(Constants.USER_DEFAULT_PASSWORD); // 默认密码
        }
        // 统一MD5加密存储
    ...
    public void updateById(Student student) {
        if (ObjectUtil.isNotEmpty(student.getPassword()) && student.getPassword().length() < 32) {
            student.setPassword(SecureUtil.md5(student.getPassword()));
        }
    ...
    public Account login(Account account) {
        ...
        if (!encryptedPswd.equals(dbStudent.getPassword())) {
