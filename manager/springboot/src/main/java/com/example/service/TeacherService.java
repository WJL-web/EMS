...
public class TeacherService {
    ...
    public void add(Teacher teacher) {
        ...
        if (ObjectUtil.isEmpty(teacher.getPassword())) {
            teacher.setPassword(Constants.USER_DEFAULT_PASSWORD); // 默认密码
        }
        // 统一MD5加密存储
    ...
    public void updateById(Teacher teacher) {
        if (ObjectUtil.isNotEmpty(teacher.getPassword()) && teacher.getPassword().length() < 32) {
            teacher.setPassword(SecureUtil.md5(teacher.getPassword()));
        }
    ...
    public Account login(Account account) {
        ...
        if (!encryptedPswd.equals(dbTeacher.getPassword())) {
